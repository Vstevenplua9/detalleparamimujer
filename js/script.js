// FileName: /detalle-novia/js/script.js
...
function createFallingGroup(){
  ...
  for(let j=0;j<iterations;j++){
    ...
    if(item.type === 'img'){
      ...
      // Ajusta el tamaño de las imágenes para que se adapten mejor a las capas de profundidad
      // Puedes ajustar estos valores (8, 20) para controlar el rango de tamaño de las imágenes
      el.style.width = `${rand(8,20)}vw`;
      el.style.height = 'auto'; // Mantener la proporción
      el.loading = 'lazy'; // Carga perezosa para mejorar el rendimiento
    } else {
      el.textContent = item.value;
      el.style.fontSize = `${sizeVW}vw`;
      el.style.webkitTextStroke = '0.0px rgba(0,0,0,0.02)'; // Pequeño trazo para mejorar la legibilidad
    }
    ...
    el.style.left = `${left}%`;
    el.style.zIndex = z - j; // Ajusta z-index para el efecto de profundidad
    ...
    el.style.userSelect = 'none'; // Evita la selección de texto/imagen
    stage.appendChild(el);
  }
  if(stage.children.length > settings.maxItems){
    // Elimina los elementos más antiguos si se excede el límite
    for(let i=0;i<6;i++){ // Elimina un pequeño lote para evitar eliminaciones constantes
